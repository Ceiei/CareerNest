...
十年前，一个人的职业身份通常是一份静态 PDF；今天，它更像一条持续变化的数据流：每天都可能新增经历、更新技能、调整方向。简历不再只是一个文件，而是一份正在生长的职业数据资产。

但现实是，每个平台、每份 JD 都在让你重新介绍一次自己。你被迫维护几十个版本的自我，却没有一个真正的源头：改一次手机号要到处同步，换一段经历要反复填写，针对不同岗位调整表达还要手工复制粘贴。

CareerNest 提供另一种解法：只维护一个真实的自己。你的个人信息、教育经历、项目、科研成果和证据，统一保存在本地唯一事实源（SSoT）中；桌面 App 负责维护这份数据资产，申请材料和浏览器插件从它读取并适配不同场景。你更新一次，其他内容随之更新；你核对一次，之后的表达都有据可循。

它管理的不是某一份简历，而是会持续生长的职业数据。

| 你要解决的问题 | CareerNest 的处理方式 |
| --- | --- |
| 反复填写不同公司的校招表单 | Chrome 插件读取本地 SSoT，扫描、匹配、预览并填写网页字段 |
| 求职和夏令营要维护两套资料 | 求职版／学术版切换，共享个人基础信息和经历，不重复录入 |
| “本科”“本科生”“学士学位”等选项容易填错 | 统一维护学历层次与学位，按标准值和同义词匹配网站下拉选项 |
| 修改一次信息却要同步几十处 | 桌面 App 是唯一编辑入口，插件只读取最新版本 |
| 简历中的成果缺少依据 | 经历可关联动作、结果、技术和证据，材料保留来源快照 |
| 每个岗位都要重新改一份简历 | 根据岗位／学校要求选择相关事实，生成可审阅的申请材料 |
| AI 改写后担心夸大或编造 | AI 只生成独立草稿，不修改事实库；材料需逐条核对来源后确认 |
| 资料和登录信息不想上传云端 | 本地优先存储，App 与插件通过本机服务连接，默认不上传资料 |

**资料只在桌面 App 编辑，Chrome 插件只负责读取、匹配、预览和填写。**
